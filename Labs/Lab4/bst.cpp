	...
	//                              Which Uses Root Ptr 
		...
		if(isEmpty())
			cout<< "No height, Empty Tree" << endl;
		else
			height(root);	
        }
 	...
 	int BinarySearchTree<Comparable>::height ( BinaryNode<Comparable> *t ) const
 	{
		...
		{
			int tempL = height(t->left)+1;
			int tempR = height(t-> right) +1;
			if(tempL < tempR)
				return tempR;
			else if(tempL > tempR)
				return 	tempL;
	...
	int BinarySearchTree<Comparable>::numLeaves( BinaryNode<Comparable> *t ) const
		...
		if(t == NULL)
			...
			return (1+ numLeaves(t -> left))+ (1+ numLeaves(t->right));


	}


	// Public call isBalance
	template <class Comparable> 
	...
	template <class Comparable>
	 bool BinarySearchTree<Comparable>:: isBalanced ( BinaryNode<Comparable> *t ) const
		...
		if(heightL == heightR || heightL -1 == heightR || heightL == heightR-1)
			return true;
